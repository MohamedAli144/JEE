...
<div class="container">
    ...
    <%
        ...
        if (utilisateur == null) {
    %>
        <div class="empty-message">Vous devez être connecté pour voir votre historique.</div>
    <%
        } else {
    ...
    for (Reservation r : reservations) {
        ...
        if (voyage != null && voyage.getTrajet() != null && voyage.getTrajet().getDate() != null) {
            ...
            java.time.LocalDate dateVoyage = dateVoyageSql.toLocalDate(); // conversion en LocalDate

            if (!dateVoyage.isAfter(aujourdHui)) { // dateVoyage <= aujourd’hui
                aDesResultats = true;
...
%>
        ...
        <%
            if (!aDesResultats) {
        %>
            <div class="empty-message">Aucune réservation passée trouvée.</div>
        <%
            }
        }
    }
